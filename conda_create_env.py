...
#
#         py_frame = ttk.Frame(self.root)
#         py_frame.pack(fill=tk.X, padx=20, pady=5)
#         ttk.Label(py_frame, text="Python 版本 (可选):").pack(side=tk.LEFT)
#         self.py_var = tk.StringVar(value="3.12")
#         py_combo = ttk.Combobox(py_frame, textvariable=self.py_var, width=10, state="normal")
#         py_combo['values'] = ("3.8", "3.9", "3.10", "3.11", "3.12", "3.13")
#         py_combo.pack(side=tk.LEFT, padx=(10, 0))
#
...
#         self.refresh_btn = ttk.Button(btn_frame, text="🔄 刷新环境列表", command=self.load_existing_envs)
...
#
#         # 右侧：已有环境
#         env_frame = ttk.LabelFrame(main_frame, text="已有虚拟环境（base 已排除）")
#         env_frame.grid(row=0, column=1, sticky="nsew", padx=(5, 0))
#
#         self.env_listbox = tk.Listbox(env_frame)
#         scrollbar = ttk.Scrollbar(env_frame, orient="vertical", command=self.env_listbox.yview)
#         self.env_listbox.configure(yscrollcommand=scrollbar.set)
#
#         self.env_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(2, 0), pady=2)
#         scrollbar.pack(side=tk.RIGHT, fill=tk.Y, pady=2)
#
#     # --- 以下方法保持不变（为节省篇幅略作精简，实际可保留原逻辑）---
#
#     def log(self, message, error=False):
#         self.log_text.config(state='normal')
#         color_tag = 'error' if error else 'info'
...
#             raise Exception("未找到 conda 命令，请确保 Anaconda 已正确安装")
#
...
#
#             env_names = []
#             self.existing_envs.clear()
#             for path in env_paths:
#                 if path == base_path:
#                     continue
#                 name = path.split("\\")[-1] if "\\" in path else path.split("/")[-1]
#                 env_names.append(name)
#                 self.existing_envs.add(name)
#
...
#
#
#     def is_valid_python_version(self, v):
#         parts = v.split('.')
#         return len(parts) == 2 and all(p.isdigit() for p in parts)
#
#     def create_env(self):
#         env_name = self.name_var.get().strip()
#         if not env_name:
#             messagebox.showwarning("输入错误", "请输入环境名称！")
#             return
#         if not env_name.replace("-", "").replace("_", "").isalnum():
#             messagebox.showwarning("输入错误", "环境名称只能包含字母、数字、下划线或连字符！")
#             return
#         if env_name in self.existing_envs:
#             messagebox.showwarning("名称冲突", f"环境 '{env_name}' 已存在！")
#             return
#
#         py_version = self.py_var.get().strip()
#         if py_version and not self.is_valid_python_version(py_version):
#             messagebox.showwarning("输入错误", "Python 版本格式应为 X.Y（如 3.10）")
#             return
#
#         cmd_args = ["create", "--name", env_name, "--yes"]
#         if py_version:
#             cmd_args.append(f"python={py_version}")
#
#         self.log(f"正在创建环境: {env_name}" + (f" (Python {py_version})" if py_version else ""))
#         self.create_btn.config(state='disabled')
#
#         thread = threading.Thread(target=self._create_in_background, args=(cmd_args, env_name))
#         thread.daemon = True
#         thread.start()
#
#     def _create_in_background(self, cmd_args, env_name):
#         try:
#             self.run_conda_cmd(cmd_args)
#             self.root.after(0, lambda: self.log(f"✅ 环境 '{env_name}' 创建成功！"))
#             self.root.after(0, lambda: messagebox.showinfo("成功", f"环境 '{env_name}' 已创建！"))
#             self.root.after(0, self.load_existing_envs)
#         except Exception as e:
#             error_msg = str(e)
#             self.root.after(0, lambda: self.log(f"❌ 创建失败: {error_msg}", error=True))
#             self.root.after(0, lambda: messagebox.showerror("错误", f"创建失败:\n{error_msg}"))
#         finally:
...
# 注意：不要保留 if __name__ == "__main__"，因为会被 main.py 调用